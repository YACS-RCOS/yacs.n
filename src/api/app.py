...
app = FastAPI()
app.add_middleware(
    SessionMiddleware, secret_key=os.environ.get("API_SIGN_KEY", "localTestingKey")
)
...
def is_admin_user(session):
    if "user" in session and (
        session["user"]["admin"] or session["user"]["super_admin"]
    ):
        return True
    return False
...
async def get_classes(
    request: Request, semester: str or None = None, search: str or None = None
):
    """
...
@cache(expire=Constants.HOUR_IN_SECONDS, coder=PickleCoder, namespace="API_CACHE")
async def get_subsemesters(
    subsemester: SubsemesterPydantic = Depends(SubsemesterPydantic),
):
    """
...
async def get_all_professors():
    ...
    (
        professors,
        error,
    ) = professor_info.get_all_professors()  # replace professor_info with db_manager
...
async def add_test_professor():
    professor, error = professor_info.add_professor(
        "random", "person", "number", "test?@rpi.edu", "CSCI", "lally 300", "52995"
    )
