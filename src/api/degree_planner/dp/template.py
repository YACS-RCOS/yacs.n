...
class Template():
    ...
    def replace_attribute(self, old_attr, new_attr):
        self.specifications = template_parsing.replace_attribute(self.specifications, old_attr, new_attr)
    ...
    def wildcard_resolutions(self, courses):
        ...
        for course in courses:
            good_match = template_parsing.course_fulfills_template(self, course)
            if good_match:
    ...
    def wildcard_differentiate(self, begin_counter):
        ...
        while self.specifications.find('*', star_index + 1) != -1:
            ...
            if star_index + 1 < len(self.specifications) and self.specifications[star_index + 1] not in template_parsing.DELIMITERS:
                continue
            self.specifications = self.specifications[:star_index + 1] + 'auto' + str(begin_counter) + self.specifications[star_index + 1:]
            begin_counter += 1
...
class template_parsing():
    ...
    def course_fulfills_template(template:Template, course:Course):
        for attr in template.specifications:
            if 'NA' in attr or 'ANY' in attr or '-1' in attr:
                continue
            if not template_parsing.evaluate_specifications(attr, course):
                return False
        return True


    @staticmethod
    def evaluate_attribute(attr:str, course:Course):
        ...
        if len(attr) and attr[0] == '$':
            ...
            truth = template_parsing.evaluate_specifications(specification, course)
            return template_parsing.evaluate_attribute(truth, course)
    ...
    @staticmethod
    def evaluate_specifications(specification:str, course:Course) -> str:
        '''
        Input -> Attribute
        Input -> True|False
        Input -> (Input)
        ...
        if '(' in specification:
            open_bracket_loc = specification.find('(')
            close_bracket_loc = len(specification) # we allow close brackets to be omitted if it's at the end of the input
            passed_bracket_count = 0

            # calculate the location of the closing bracket for the current bracket
            for i in range(open_bracket_loc + 1, len(specification)):
                if specification[i] == '(':
            ...
            new_string = specification[: open_bracket_loc] + str(template_parsing.evaluate_specifications(specification[open_bracket_loc + 1 : close_bracket_loc], course)) + specification[close_bracket_loc + 1:]
            return template_parsing.evaluate_specifications(new_string, course)
        
        if '&' in specification:
            and_loc = specification.find('&')
            return template_parsing.evaluate_specifications(specification[: and_loc], course) and template_parsing.evaluate_specifications(specification[and_loc + 1:], course)
        
        if '|' in specification:
            and_loc = specification.find('|')
            return template_parsing.evaluate_specifications(specification[: and_loc], course) or template_parsing.evaluate_specifications(specification[and_loc + 1:], course)

        if len(specification) and specification[0] == '~':
            return not template_parsing.evaluate_specifications(specification[1:], course)

        truth = template_parsing.evaluate_attribute(specification, course)
        return truth
    

    @staticmethod
    def replace_attribute(specifications, search_attr, target_attr):
        print('old specification: ' + specifications)
        search_begin_index = 0
        while specifications.find(search_attr, search_begin_index) != -1:
            begin_index = specifications.find(search_attr)
            end_index = af.find_set(specifications, template_parsing.DELIMITERS, begin_index + 1)
            if end_index != -1:
                end_index += 0
            else:
                end_index = len(specifications)
            search_begin_index = end_index + len(target_attr) - (end_index - search_begin_index)
            specifications = specifications[:begin_index] + target_attr + specifications[end_index:]
        
        print('new specification: ' + specifications)
