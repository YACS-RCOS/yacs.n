<template>
  <b-container fluid class="py-3 h-100">
    <b-row class="h-100">
      <b-col md="4" class="d-flex flex-column px-0">
        ...
        <CourseList
          v-else
          class="w-100"
          :active-course-sections="activeSections"
        />
        <!-- </b-card-text>
            </b-tab>
      ...
      <div id="allScheduleData" class="col-md-8">
        <CourseSchedule
          :courses="selectedCourses"
          :course-sections="selectedCourseSections"
          @changeActiveSections="
            activeSections.splice(0, activeSections.length, ...$event)
          "
        />
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("schedule", ["selectedCourses", "selectedCourseSections"]),
    // courses() {
     ...
     */
    selectedCrns() {
      return this.activeSections.map((section) => section.crn).join(", ");
     ...
     */
    totalCredits() {
      const array = this.$store.state.schedule.selectedCourseIds.map(
        (courseId) => this.$store.getters.courseById(courseId).max_credits
      );
    ...
    numSelectedCourses() {
      return this.$store.state.schedule.selectedCourseIds.length;
  ...
  methods: {
    ...
    changeSelectedScheduleIndex(newIndex) {
